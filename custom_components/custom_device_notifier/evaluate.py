...
async def evaluate_condition(hass: HomeAssistant, cfg: Mapping[str, Any]) -> bool:
    ...
    for eid in entity_ids:
        ...
        if value in ["unknown", "unavailable", "unknown or unavailable"]:
            ...
            template_str = f"{{{{ states('{eid}') in ['unknown', 'unavailable'] }}}}" if value == "unknown or unavailable" else f"{{{{ is_state('{eid}', '{value}') }}}}"
            if operator == "!=":
            ...
            if not is_numeric:
                ...
                if operator == "!=":
                    ha_cfg = {"condition": "template", "value_template": f"{{{{ not is_state('{eid}', '{value}') }}}}"}
            else:
                if operator == "==":
                    ha_cfg = {"condition": "numeric_state", "entity_id": eid, "value": str(value)}
                elif operator == ">":
                    ha_cfg = {"condition": "numeric_state", "entity_id": eid, "above": str(value)}
                elif operator == "<":
                    ha_cfg = {"condition": "numeric_state", "entity_id": eid, "below": str(value)}
                elif operator == ">=":
                    ha_cfg = {"condition": "numeric_state", "entity_id": eid, "above": str(float(value) - 0.0001)}
                elif operator == "<=":
                    ha_cfg = {"condition": "numeric_state", "entity_id": eid, "below": str(float(value) + 0.0001)}
                elif operator == "!=":
                    ha_cfg = {"condition": "template", "value_template": f"{{{{ states('{eid}') | float != {value} | float }}}}"}
                else:
