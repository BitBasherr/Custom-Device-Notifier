...
async def evaluate_condition(hass: HomeAssistant, cfg: Mapping[str, Any]) -> bool:
    ...
    if value in ["unknown", "unavailable", "unknown or unavailable"]:
        ...
        template_str = f"{{{{ states('{entity_id}') in ['unknown', 'unavailable'] }}}}" if value == "unknown or unavailable" else f"{{{{ is_state('{entity_id}', '{value}') }}}}"
        if operator == "!=":
        ...
        if not is_numeric:
            ...
            if operator == "!=":
                ha_cfg = {"condition": "template", "value_template": f"{{{{ not is_state('{entity_id}', '{value}') }}}}"}
        else:
            if operator == "==":
                ha_cfg = {"condition": "numeric_state", "entity_id": entity_id, "value": value}
            elif operator == ">":
                ha_cfg = {"condition": "numeric_state", "entity_id": entity_id, "above": value}
            elif operator == "<":
                ha_cfg = {"condition": "numeric_state", "entity_id": entity_id, "below": value}
            elif operator == ">=":
                ha_cfg = {"condition": "numeric_state", "entity_id": entity_id, "above": float(value) - 0.0001}  # Approximate >=
            elif operator == "<=":
                ha_cfg = {"condition": "numeric_state", "entity_id": entity_id, "below": float(value) + 0.0001}  # Approximate <=
            elif operator == "!=":
