...
def _pc_usable(
    hass: HomeAssistant, session_sensor: Optional[str], fresh_s: int
) -> bool:
...
def _phone_usable(
    ...
    *,
    ...
    if require_awake and _safe(trig) in ("", SHUTDOWN, SCREEN_OFF):
        return False
    if (
        require_unlocked and _safe(locked) != "off"
    ):  # your lock sensors: off == unlocked
        return False
    try:
    ...
    def first_phone() -> Optional[str]:
        for sp in specs:
            if _phone_usable(
                hass,
                sp,
                min_batt=min_battery,
                fresh_s=phone_fresh_s,
