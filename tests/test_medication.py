...
def test_log_medication_taken_creates_new_file(mock_exists, mock_file, mock_hass):
    ...
    # Verify write was called (would write header + data)
    handle = mock_file()
...
async def test_medication_sensor_mark_taken(mock_hass, mock_entry):
    ...
    # Mock the executor job
    mock_hass.async_add_executor_job = Mock(return_value=None)
    
    with patch("custom_components.custom_device_notifier.medication_sensor.dt_util.now", return_value=test_time):
        await sensor.async_mark_taken(test_time)
...
async def test_medication_sensor_prevents_duplicate_doses(mock_hass, mock_entry):
    ...
    with patch("custom_components.custom_device_notifier.medication_sensor.dt_util.now", return_value=time1):
        await sensor.async_mark_taken(time1)
        await sensor.async_mark_taken(time2)
