...
async def test_full_config_flow(hass):
    ...
    called = []
    async def _noop(call): called.append(call)
    ...
    # Name
    result = await hass.config_entries.flow.async_configure(result["flow_id"], {"service_name_raw": "My Notifier"})
    assert result["type"] == data_entry_flow.RESULT_TYPE_FORM
    assert result["step_id"] == "add_target"

    # Add target
    result = await hass.config_entries.flow.async_configure(result["flow_id"], {"target_service": "mobile_app_pixel"})
    ...
    # Pick entity
    result = await hass.config_entries.flow.async_configure(result["flow_id"], {"entity": "sensor.pixel_battery"})
    ...
    # Require all
    result = await hass.config_entries.flow.async_configure(result["flow_id"], {"match_mode": "all"})
    ...
    result = await hass.config_entries.flow.async_configure(result["flow_id"], {"priority": priority})
    assert result["step_id"] == "choose_fallback"

    # Choose fallback and finish
    result = await hass.config_entries.flow.async_configure(result["flow_id"], {"fallback": "mobile_app_pixel"})
...
async def test_back_from_fallback(hass):
    ...
    res = await hass.config_entries.flow.async_configure(res["flow_id"], {"service_name_raw": "My Notifier"})
    res = await hass.config_entries.flow.async_configure(res["flow_id"], {"target_service": "mobile_app_pixel"})
    res = await hass.config_entries.flow.async_configure(res["flow_id"], {"choice": "done"})
    res = await hass.config_entries.flow.async_configure(res["flow_id"], {"match_mode": "all"})
    res = await hass.config_entries.flow.async_configure(res["flow_id"], {"next": "done"})
    res = await hass.config_entries.flow.async_configure(res["flow_id"], {"priority": ["notify.mobile_app_pixel"]})
    assert res["step_id"] == "choose_fallback"

    # Back (via nav selector)
    res = await hass.config_entries.flow.async_configure(res["flow_id"], {"fallback": "mobile_app_pixel", "nav": "back"})
